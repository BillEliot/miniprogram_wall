...
Page({
  ...
  onComment: function () {
    if (app.globalData.user.uid == -1) {
      ...
      let _this = this
      wx.request({
        url: app.globalData.url + '/api/submitDealComment',
        method: 'post',
        header: {
        ...
        success: function (res) {
          if (res.data == 1) {
            ...
            _this.data.deal.comments = newComment.concat(_this.data.deal.comments);
            _this.setData({
              comment: '',
              'deal.comments': _this.data.deal.comments
            })
  ...
  onLoad: function (options) {
    ...
    let _this = this
    wx.request({
      url: app.globalData.url + '/api/getDealDetail',
      method: 'post',
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },
